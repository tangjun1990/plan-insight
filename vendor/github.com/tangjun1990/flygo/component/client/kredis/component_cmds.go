...
func (r *Component) Get(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		return reply, fmt.Errorf("kredis get string error %w", err)
	}
	return reply, err
}
...
func (r *Component) GetBytes(ctx context.Context, key string) ([]byte, error) {
	...
	if err != nil {
		return b, fmt.Errorf("kredis get bytes error %w", err)
	}
	return b, nil
}
...
func (r *Component) MGetString(ctx context.Context, keys ...string) ([]string, error) {
	...
	if err != nil {
		return []string{}, fmt.Errorf("kredis mgetstring error %w", err)
	}
...
func (r *Component) HMGetMap(ctx context.Context, key string, fields []string) (map[string]string, error) {
	if len(fields) == 0 {
		return make(map[string]string), fmt.Errorf("kredis hmgetmap error %w", ErrInvalidParams)
	}
	reply, err := r.client.HMGet(ctx, key, fields...).Result()
	if err != nil {
		return make(map[string]string), fmt.Errorf("kredis hmgetmap error %w", err)
	}
...
func (r *Component) HMSet(ctx context.Context, key string, hash map[string]interface{}, expire time.Duration) error {
	if len(hash) == 0 {
		return fmt.Errorf("kredis hmset error %w", ErrInvalidParams)
	}
	...
	if expire > 0 {
		...
		if err != nil {
			return fmt.Errorf("kredis hmset expire error %w", err)
		}
	}
	return nil
}
